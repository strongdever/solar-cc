<script>
    $(function() {
        ...
        searchform.submit(function(e) {
            ...
            let noResulsHtml = '<tr>' +
                                '<td colspan="9">{!! trans("usersmanagement.search.no-results") !!}</td>' +
                                '</tr>';
                ...
                success: function (result) {
                    ...
                    if (jsonData.length != 0) {
                        $.each(jsonData, function(index, val) {
                            let rolesHtml = '';
                            let roleClass = '';
                            let showCellHtml = '<a class="btn btn-sm btn-success btn-block" href="users/' + val.id + '">{!! trans("usersmanagement.buttons.show") !!}</a>';
                            let editCellHtml = '<a class="btn btn-sm btn-info btn-block" href="users/' + val.id + '/edit">{!! trans("usersmanagement.buttons.edit") !!}</a>';
                            let deleteCellHtml = '<form method="POST" action="/users/'+ val.id +'" accept-charset="UTF-8">' +
                                    '{!! Form::hidden("_method", "DELETE") !!}' +
                                    '{!! csrf_field() !!}' +
                                    '<button class="btn btn-sm btn-danger btn-block" type="button" data-toggle="modal" data-target="#confirmDelete" data-title="{!! trans("usersmanagement.modals.delete_title") !!}" data-message="{!! trans("usersmanagement.modals.delete_message") !!}">' +
        ...
        searchformInput.keyup(function(event) {
            if ($('#user-search-input').val() != '') {
                ...
                resultsContainer.html('');
                usersTable.show();
                cardTitle.html("{!! trans('usersmanagement.menu-alt') !!}");
                userPagination.show();
                usersCount.html(" ");
            };
        });
